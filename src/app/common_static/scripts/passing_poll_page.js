...
questionsList.forEach(question => {
    ...
    const questionEl = $(`<div id="${question.id}"></div>`)
...
function answerType(questionType, questionId, question) {
    ...
    if (questionType == "short text") {
        return '<input class="answerShort" type="text" maxlength="60" placeholder="введите ответ">';
    } else if (questionType == "long text") {
        return '<textarea class="answerLong" type="text" maxlength="60" placeholder="введите ответ">';

    } else if (questionType == "radiobutton" | questionType == "checkbox") {
        // name равно индексу question, id должно быть уникально как в кажодой радиокнопке, так и в каждом вопросе, поэтому оно будет составляться из id вопроса и номера кнопки {question_id}-{radiobutton_id}. номер кнопки из counter считаем, просто по порядку
